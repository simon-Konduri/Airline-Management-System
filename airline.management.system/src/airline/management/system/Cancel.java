...
public class Cancel extends JFrame { //Sixth
	...
	add(textField_4);
		...
		try{	
                    ...
                    //first check wheather ticket - id is in reservation table or not
                    PreparedStatement check = c.c.prepareCall("select * from reservation where ticket_id = ?");
                    check.setString(1, ticket_id);
                    ResultSet rs = check.executeQuery();
                    if(rs.next()){
                        PreparedStatement stmt = c.c.prepareCall("delete from reservation where ticket_id = ?");
                        stmt.setString(1, ticket_id);
                        stmt.executeUpdate();
                        c.s.executeUpdate(str);
                    }
                    else{
                        JOptionPane.showMessageDialog(null,"Ticket id is not valid");
                        return;
                    }
